...
def solvebs(f, a, b, tol, verbose):
    if  not isbsAble(f, a, b):
        return None
    k = -1*int(1+(np.log10(tol)/np.log10(b-a))/np.log10(2))
    if verbose:
...
if __name__=='__main__':
    ...
    print(solvenewton(f, fprime, 0.1, .0000001, 200, False))
    ...
    #bisection test
    print("Task 4:")
    print(solvebs(f, -0.5, 0.85, .00001, True))
    ...
    f2_roots = []
    for i in x0_f2:
        x = solvefp(f_2, i, .001, 200, (verbose))
        if x!=None:
            f2_roots.append(x)
